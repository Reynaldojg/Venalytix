    ...
    public class CsvExtractor : IExtractor
    {
        ...
        public async Task<OperationResult> ExtraerAsync()
        {
            try
            {
                ...
                // 1️⃣ Leemos las rutas desde el appsettings.json
                ...
                // 4️⃣ Guardar JSON de salida
                var resultado = new { Clientes = clientes, Productos = productos, Ventas = ventas };
                var jsonPath = Path.Combine(outputPath, "resultado_extraccion.json");

                await File.WriteAllTextAsync(
                    jsonPath,
                    JsonSerializer.Serialize(resultado, new JsonSerializerOptions { WriteIndented = true })
                );

                _logger.LogInformation("✅ Extracción completada. Archivo JSON generado en {0}", jsonPath);
                return OperationResult.Success(resultado, "Extracción CSV completada exitosamente.");
            }
            catch (Exception ex)
            {
